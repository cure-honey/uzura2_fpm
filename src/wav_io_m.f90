    ...
    contains
        ...
        subroutine pcm1frame(this, pcm) ! copy from array
            ...
            case (1) !mono
                pcm(1152:1:-1, 1) = real( this%pcm16(ipos:ipos + 1152 - 1), kind = kd ) / 2**15 
                ipos = ipos + 1152
            case (2) !stereo
                pcm(1152:1:-1, 1) = real( this%pcm16(ipos    :ipos + 2 * 1152 - 1:2), kind = kd ) / 2**15 ! L 16bit int
                pcm(1152:1:-1, 2) = real( this%pcm16(ipos + 1:ipos + 2 * 1152 - 1:2), kind = kd ) / 2**15 ! R
                ipos = ipos + 2 * 1152
            case default
